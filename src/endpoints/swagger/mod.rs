...
fn get_operation_and_schema(
    ...
    if schema_res.is_err() {
        warn!(
            "Found declaration in {}.sql, but it's maleformatted",
            endpoint.url_path
        );
        return (operation.build(), schema_openapi_map);
    }
    ...
    if let Some(description_val) = declaration.get("description") {
        if !description_val.is_string() {
            warn!(
                "Found declaration in {}.sql, declaration.description must be a string",
                endpoint.url_path
            );
        }
    ...
    if let Some((response, schema)) = get_response(&declaration) {
        ...
        schema_openapi_map.insert(
            format!("Response{}", endpoint.url_path.replace("/", "_")).to_case(Case::UpperCamel),
            schema,
        );
    }
    ...
    if endpoint.method == EndpointMethod::GET {
        ...
        if let Some((request_body, schema)) = get_request_body(&declaration) {
            ...
            schema_openapi_map.insert(
                format!("Post{}", endpoint.url_path.replace("/", "_")).to_case(Case::UpperCamel),
                schema,
            );
        }
    }
    ...
    for project in &collection.projects {
        for endpoint in &project.endpoints {
            (paths_builder, components_builder) =
                build_schema_for_endpoint(paths_builder, components_builder, &endpoint, &project);
        }
    }
